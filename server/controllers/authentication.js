...
// Find user, if it's not found will just finish request for safe reasons
// Create recovery account token
// send email assynk, will not wait
function forgot(req, res, next) {
    ...
    if (!user.email) {
        next.({ status: 400, content: 'Você precisa informar seu e-mail'});
        return;
    }
    ...
    userRepository.findOne(user).then(userFound) => {
        ...
        const emailConfig = {
          to: user.email,
          subject: '[FilmesFlash] - Recuperar senha'
        };
...
function register(req, res, next) {
    ...
    function sendWelcome() {
        ...
        if (user.email) {
            const emailConfig = {
              to: user.email,
              subject: `[FilmesFlash] - Bem-Vindo ${name}`
            };
