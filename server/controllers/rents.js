...
// Check on if movie is available
// Verify user id to populate rents history
...
// Come across user id to populate rents history
...
function returnMovie(req, res, next) {
  ...
  userRepository.findByEmail(req.decoded.email).then((userFound) => {
    rentRepository.returnMovie(req.body.movie, userFound.id).then((data) => {
      if (!data || data.changeRows === 0) {
        next({ status: 400, content: 'Não encontrada locação para esta devolução' });
      }
      else{
